...
#define SampleIntervalSeconds 0 // this will be ignored unless SampleIntervalMinutes=0
...
void setup() {
  ...
  pinMode(RTC_INTERRUPT_PIN,INPUT_PULLUP);//RTC alarms low, so you might need pullup on the D2 line
  //Note: the above line is not needed if you have hardware pullup resistors on the SQW line, and many RTC modules do.
...
else  //for sub-minute alarms use the full setA1time function // ONLY occurs when SampleIntervalMinutes <=0
        ...
        {  //error catch - if alarmminute=60 the interrupt never triggers due to rollover!
          ...
          if (Alarmhour > 23) { //uhoh a day rollover, but we dont know the month..so we dont know the next day number?
            Alarmhour =0;      
             ...
             DateTime now = RTC.now(); // the LONG delay to force a midnight rollover requires a second time reading:
             Alarmhour = now.hour();
             Alarmminute = now.minute() + SampleIntervalMinutes;
             Alarmsecond = now.second() + SampleIntervalSeconds;
             Alarmday = now.day();
